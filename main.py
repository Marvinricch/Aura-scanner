...
RSI14_PERIOD = 14
HARD_RSI14_BUY_MAX = float(os.getenv("HARD_RSI14_BUY_MAX", "68"))   # discard BUY if RSI(14) already this extended
HARD_RSI14_SELL_MIN = float(os.getenv("HARD_RSI14_SELL_MIN", "32"))  # discard SELL if RSI(14) already this extended (mirror of the BUY case)
MIN_TP_GAP_PCT = float(os.getenv("MIN_TP_GAP_PCT", "0.5"))   # TP2 must clear TP1 by at least this % of entry price, else TP2 is dropped
...
WHALE_LS_WEAK_THRESHOLD = float(os.getenv("WHALE_LS_WEAK_THRESHOLD", "1.2"))  # whale buy:sell ratio below this on a BUY = weak conviction
...
DAILY_CONTEXT_CACHE_SECONDS = int(os.getenv("DAILY_CONTEXT_CACHE_SECONDS", "43200"))  # 12h — OI history and daily structure change slowly, no need to refetch every cycle
...
async def get_daily_context(symbol: str) -> dict:
    """OI-30-days-ago, 7-day average volume, and nearest daily resistance/
    support — all change slowly, so this is cached for hours rather than
    refetched every scan cycle. A failure here degrades to missing fields
    (the quality gate just skips those specific flags/points), never a crash."""
    cached = DAILY_CONTEXT_CACHE.get(symbol)
    if cached and (time.time() - cached["fetched_at"]) < DAILY_CONTEXT_CACHE_SECONDS:
        return cached

    result = {"fetched_at": time.time(), "oi_30d_ago": None, "avg_volume_7d": None,
              "daily_resistance": None, "daily_support": None}
    try:
...
def evaluate_signal_quality(snap, rsi14: float, funding_rate: float | None,
                             oi_30d_ago: float | None, avg_volume_7d: float | None,
                             volume_24h: float | None, daily_resistance: float | None,
                             daily_support: float | None, whale_ls_ratio: float | None) -> dict:
    ...
    if direction == "BUY" and rsi14 >= HARD_RSI14_BUY_MAX:
        return {"passed": False, "flags": [], "score": 0, "drop_tp2": False, "discard": True}
    ...
    tp1_rr = None
    if snap.stop_loss is not None and snap.take_profit is not None:
        risk = abs(snap.price - snap.stop_loss)
        reward = abs(snap.take_profit - snap.price)
        if risk > 0:
            tp1_rr = reward / risk
    if tp1_rr is None or tp1_rr < MIN_TP1_RR:
        return {"passed": False, "flags": [], "score": 0, "drop_tp2": drop_tp2, "discard": True}
    ...
    if oi_30d_ago and oi_30d_ago > 0 and snap.open_interest is not None:
        oi_change_pct = (snap.open_interest - oi_30d_ago) / oi_30d_ago * 100
...
async def build_snapshot(symbol: str, ticker_24h: dict | None) -> SymbolSnapshot | None:
    ...
    if snap.signal in ("BUY", "SELL"):
        ...
        quality = evaluate_signal_quality(
            ...
            daily_ctx["oi_30d_ago"], daily_ctx["avg_volume_7d"], volume_24h,
...
def format_signal_alert(symbol: str, signal: str, price: float, rsi: float,
                         ...
                         score: int = 0, flags: list[str] | None = None) -> str:
...
DAILY_CONTEXT_CACHE: dict[str, dict] = {}  # symbol -> {"fetched_at", "oi_30d_ago", "avg_volume_7d", "daily_resistance", "daily_support"} — refreshed rarely, these change slowly
...
async def process_symbol(symbol: str, ticker_24h: dict | None):
    ...
    if snap.signal != prev_signal and snap.signal in ("BUY", "SELL"):
        text = format_signal_alert(symbol, snap.signal, snap.price, snap.rsi, snap.long_pct,
                                    ...
                                    snap.signal_score, snap.signal_flags)
