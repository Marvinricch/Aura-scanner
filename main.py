...
BINANCE_CONCURRENCY = int(os.getenv("BINANCE_CONCURRENCY", "3"))  # max simultaneous in-flight Binance requests, across all symbols
_binance_semaphore = asyncio.Semaphore(BINANCE_CONCURRENCY)
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code == 429 or resp.status_code == 418:
                # rate limited — back off. 418 is Binance's harder IP-level
                # block (can outlast our own backoff), 429 is the softer
                # per-minute limit — track which one we're actually hitting.
...
async def get_klines(symbol: str, interval: str = None, limit: int = None):
    ...
    return data  # None = request genuinely failed (rate limit/network); [] or short list = real data from Binance
...
async def build_snapshot(symbol: str, ticker_24h: dict | None) -> SymbolSnapshot | None:
    ...
    if klines is None:
        raise RuntimeError("Binance request failed (rate limited or network error) — will retry next cycle")
    if len(klines) < PIVOT_LOOKBACK * 2 + 5:
        return None  # genuinely too little history from Binance — not a retryable failure
