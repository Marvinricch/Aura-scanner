...
CORE_GATE_WHALE_BUY_MIN = float(os.getenv("CORE_GATE_WHALE_BUY_MIN", "1.5"))     # whale buy:sell ratio must exceed this for a BUY
CORE_GATE_WHALE_SELL_MAX = float(os.getenv("CORE_GATE_WHALE_SELL_MAX", "0.7"))   # whale buy:sell ratio must be below this for a SELL
...
def evaluate_signal_quality(snap, rsi14: float, funding_rate: float | None,
    ...
    if direction == "BUY":
        if whale_ls_ratio is None or whale_ls_ratio <= CORE_GATE_WHALE_BUY_MIN:
            return _reject(f"Whale ratio {_ratio(whale_ls_ratio)} does not exceed required "
                            f">{CORE_GATE_WHALE_BUY_MIN}:1 for long")
        if fund_side != "Bullish":
            return _reject(f"Fund side is {fund_side or 'unknown'}, required Bullish for long")
        if snap.pct_change_24h is not None and snap.pct_change_24h > CORE_GATE_PRICE_CHG_MAX_PCT:
            return _reject(f"24h change {_pct(snap.pct_change_24h)} — overextended, late entry risk")
        ...
        if primary_ok:
            ...
            if not fallback_ok:
                return _reject("No momentum confirmed (24h primary and 8h/1h fallback both missed)")
            momentum_basis = "FALLBACK"
        if vol_ratio_pct is None or vol_ratio_pct <= CORE_GATE_VOLUME_MIN_PCT:
            return _reject(f"Volume {_pct(vol_ratio_pct)} of 7d avg, required >{CORE_GATE_VOLUME_MIN_PCT}%")
        if oi_24h_change_pct is not None and oi_24h_change_pct <= CORE_GATE_OI_24H_MIN_PCT:
            return _reject(f"OI 24h change {_pct(oi_24h_change_pct)}, required >{CORE_GATE_OI_24H_MIN_PCT}%")
        elif oi_24h_change_pct is None:
            flags.append("⚠️ OI UNVERIFIED — 24h OI change could not be confirmed")
    elif direction == "SELL":
        if whale_ls_ratio is None or whale_ls_ratio >= CORE_GATE_WHALE_SELL_MAX:
            return _reject(f"Whale ratio {_ratio(whale_ls_ratio)} does not fall below required "
                            f"<{CORE_GATE_WHALE_SELL_MAX}:1 for short")
        if fund_side != "Bearish":
            return _reject(f"Fund side is {fund_side or 'unknown'}, required Bearish for short")
        if snap.pct_change_24h is not None and snap.pct_change_24h < -CORE_GATE_PRICE_CHG_MAX_PCT:
            return _reject(f"24h change {_pct(snap.pct_change_24h)} — overextended, late entry risk")
        primary_ok = snap.pct_change_24h is not None and \
