...
def detect_triple_touch(df: pd.DataFrame, is_high: pd.Series, is_low: pd.Series,
                         direction: str, lookback: int = None) -> tuple[float | None, int]:
    ...
    Returns (level, touch_count); level is None if nothing clears the
    minimum touch count."""
    ...
    for v in values:
        ...
        count = int(np.sum(np.abs(values - v) <= band))
        if count > best_count:
...
def _compute_indicators(klines: list) -> dict:
    ...
    resistance_level, resistance_touches = detect_triple_touch(df, is_high, is_low, "resistance")
    support_level, support_touches = detect_triple_touch(df, is_high, is_low, "support")
