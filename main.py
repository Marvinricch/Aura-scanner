...
PRICE_PAST_ENTRY_PCT = float(os.getenv("PRICE_PAST_ENTRY_PCT", "0.5"))  # current price this far (%) beyond the confirmed retracement-zone entry triggers the pullback check below
PULLBACK_SUPPORT_TOLERANCE_PCT = float(os.getenv("PULLBACK_SUPPORT_TOLERANCE_PCT", "1.0"))  # a structural level within this % of current price counts as a usable adjusted entry
...
CORE_GATE_PRICE_CHG_MAX_PCT = float(os.getenv("CORE_GATE_PRICE_CHG_MAX_PCT", "10.0"))   # max |24h % change| allowed, both directions
CORE_GATE_VOLUME_MIN_PCT = float(os.getenv("CORE_GATE_VOLUME_MIN_PCT", "70"))    # 24h volume must exceed this % of the 7d average
CORE_GATE_OI_24H_MIN_PCT = float(os.getenv("CORE_GATE_OI_24H_MIN_PCT", "-15"))   # OI 24h change must exceed this (i.e. not have collapsed past it)
CORE_GATE_ENTRY_GAP_MAX_PCT = float(os.getenv("CORE_GATE_ENTRY_GAP_MAX_PCT", "0.5"))  # entry price must stay within this % of current live price
...
class SymbolSnapshot:
    __slots__ = (
        "symbol", "price", "entry_price", "pct_change_24h", "rsi", "ema_fast", "ema_slow",
...
def evaluate_signal_quality(snap, rsi14: float, funding_rate: float | None,
                             ...
                             whale_ls_ratio: float | None, funding_rate_history: list | None = None) -> dict:
    ...
    entry_gap_pct = (abs(entry - snap.price) / snap.price * 100) if (snap.price and entry) else None
    ...
    # meant to be a hard go/no-go, not best-effort. ----
    if direction == "BUY":
        ...
        if snap.pct_change_24h is None or not (CORE_GATE_PRICE_CHG_MIN_PCT <= snap.pct_change_24h <= CORE_GATE_PRICE_CHG_MAX_PCT):
            return _reject(f"24h change {_pct(snap.pct_change_24h)} outside required "
                            f"+{CORE_GATE_PRICE_CHG_MIN_PCT}%..+{CORE_GATE_PRICE_CHG_MAX_PCT}% range for long")
        ...
        if oi_24h_change_pct is None or oi_24h_change_pct <= CORE_GATE_OI_24H_MIN_PCT:
            return _reject(f"OI 24h change {_pct(oi_24h_change_pct)}, required >{CORE_GATE_OI_24H_MIN_PCT}%")
        if entry_gap_pct is None or entry_gap_pct > CORE_GATE_ENTRY_GAP_MAX_PCT:
            return _reject(f"Entry {_pct(entry_gap_pct)} from current price, exceeds "
                            f"{CORE_GATE_ENTRY_GAP_MAX_PCT}% max")
    elif direction == "SELL":
        ...
        if snap.pct_change_24h is None or not (-CORE_GATE_PRICE_CHG_MAX_PCT <= snap.pct_change_24h <= -CORE_GATE_PRICE_CHG_MIN_PCT):
            return _reject(f"24h change {_pct(snap.pct_change_24h)} outside required "
                            f"-{CORE_GATE_PRICE_CHG_MAX_PCT}%..-{CORE_GATE_PRICE_CHG_MIN_PCT}% range for short")
        if vol_ratio_pct is None or vol_ratio_pct <= CORE_GATE_VOLUME_MIN_PCT:
            return _reject(f"Volume {_pct(vol_ratio_pct)} of 7d avg, required >{CORE_GATE_VOLUME_MIN_PCT}%")
        if oi_24h_change_pct is None or oi_24h_change_pct <= CORE_GATE_OI_24H_MIN_PCT:
            return _reject(f"OI 24h change {_pct(oi_24h_change_pct)}, required >{CORE_GATE_OI_24H_MIN_PCT}%")
    ...
    if direction == "BUY" and rsi14 >= HARD_RSI14_BUY_MAX:
        return _reject(f"RSI14 {rsi14:.1f} at/above hard ceiling {HARD_RSI14_BUY_MAX}")
    if direction == "SELL" and rsi14 <= HARD_RSI14_SELL_MIN:
        return _reject(f"RSI14 {rsi14:.1f} at/below hard floor {HARD_RSI14_SELL_MIN}")
    ...
    if direction == "BUY" and whale_ls_ratio is not None and whale_ls_ratio < WHALE_HARD_BLOCK_BUY_MIN:
        return _reject(f"Whale flow too short for long ({whale_ls_ratio:.2f}:1)")
    if direction == "SELL" and whale_ls_ratio is not None and whale_ls_ratio > WHALE_HARD_BLOCK_SELL_MAX:
        return _reject(f"Whale flow too long for short ({whale_ls_ratio:.2f}:1)")
    ...
    if direction == "BUY" and fund_side == "Bearish" and whale_ls_ratio is not None and whale_ls_ratio < 1.0:
        return _reject("Fund flow and whale flow both bearish")
    if direction == "SELL" and fund_side == "Bullish" and whale_ls_ratio is not None and whale_ls_ratio > 1.2:
        return _reject("Fund flow and whale flow both bullish")
    ...
    if direction == "BUY" and BORDERLINE_RSI14_BUY_MIN <= rsi14 < HARD_RSI14_BUY_MAX:
        ...
        if not ok:
            return _reject(f"Borderline RSI14 ({rsi14:.1f}) requires whale_ratio >= "
                            f"{BORDERLINE_WHALE_LS_BUY_MIN} and bullish fund flow")
    if direction == "SELL" and HARD_RSI14_SELL_MIN < rsi14 <= BORDERLINE_RSI14_SELL_MAX:
        ...
        if not ok:
            return _reject(f"Borderline RSI14 ({rsi14:.1f}) requires whale_ratio <= "
                            f"{BORDERLINE_WHALE_LS_SELL_MAX} and bearish fund flow")
    ...
    if len(funding_rate_history) >= 10:
        ...
        if len(unique_values) <= FUNDING_LOCK_HARD_MAX_UNIQUE:
            return _reject("Funding rate locked — illiquid environment")
    ...
    return {"passed": score >= SIGNAL_SCORE_ALERT_MIN, "flags": flags, "score": score,
            "drop_tp2": drop_tp2, "discard": False}


def apply_entry_gap_check(snap: SymbolSnapshot, computed: dict) -> str | None:
    """Price-past-entry guard. Compares live price against the confirmed
    retracement-zone boundary (the actual level the pipeline confirmed on),
    not against itself — snap.price and the "entry" used to be treated as
    identical, which is exactly how signals were dispatching 1-2% past a
    usable entry. Returns a rejection reason string if there's no nearby
    pullback level to adjust into, or if the adjusted R:R falls below 2.0.
    On success, mutates snap.entry_price in place (snap.price is left
    untouched — it's still the true live price used for TP-hit/trailing
    logic elsewhere) and returns None."""
    direction = snap.signal
    if direction not in ("BUY", "SELL") or snap.stop_loss is None or snap.take_profit is None or not snap.price:
        return None
    ...
    if direction == "BUY":
        ...
        price_gap = (snap.price - entry_ref) / entry_ref
        if price_gap <= PRICE_PAST_ENTRY_PCT / 100:
            return None
        support = computed.get("target_below")
        support_ok = support is not None and support > 0 and support < snap.price and \
            (snap.price - support) / snap.price * 100 <= PULLBACK_SUPPORT_TOLERANCE_PCT
        if not support_ok:
            return "Price past entry, no pullback level found"
        new_entry = support
        risk = new_entry - snap.stop_loss
        if risk <= 0:
            return "Price past entry, no pullback level found"
        if (snap.take_profit - new_entry) / risk < 2.0:
            return "R:R below 2.0 after entry adjustment"
        snap.entry_price = new_entry
        return None
    else:
        entry_ref = computed.get("bear_entry_ref")
        if not entry_ref or entry_ref <= 0:
            return None
        price_gap = (entry_ref - snap.price) / entry_ref
        if price_gap <= PRICE_PAST_ENTRY_PCT / 100:
            return None
        resistance = computed.get("target_above")
        resistance_ok = resistance is not None and resistance > 0 and resistance > snap.price and \
            (resistance - snap.price) / snap.price * 100 <= PULLBACK_SUPPORT_TOLERANCE_PCT
        if not resistance_ok:
            return "Price past entry, no pullback level found"
        new_entry = resistance
        risk = snap.stop_loss - new_entry
        if risk <= 0:
            return "Price past entry, no pullback level found"
        if (new_entry - snap.take_profit) / risk < 2.0:
            return "R:R below 2.0 after entry adjustment"
        snap.entry_price = new_entry
        return None
...
async def build_snapshot(symbol: str, ticker_24h: dict | None) -> SymbolSnapshot | None:
    ...
    if snap.signal in ("BUY", "SELL"):
        # ---- price-past-entry check runs first: either rejects outright,
        # or moves snap.entry_price to an adjusted pullback level before
        # anything else (RR, scoring) is computed against it ----
        ...
        quality = evaluate_signal_quality(
            ...
            daily_ctx["funding_rate_history"],
        )
...
def format_signal_alert(symbol: str, signal: str, price: float, rsi: float,
    ...
    if entry_price is not None and abs(entry_price - price) > 1e-12:
        # entry was adjusted off current price by the price-past-entry check —
        # show both so it's clear this isn't a market-price entry
        lines.append(f"Entry: {_fmt_price(entry)} (adjusted — price was {_fmt_price(price)})")
    else:
